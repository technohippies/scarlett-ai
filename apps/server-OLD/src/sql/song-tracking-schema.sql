-- Additional tables for song match tracking and analytics
...
CREATE TABLE song_match_events (
  id TEXT PRIMARY KEY, -- UUID
...
CREATE INDEX idx_song_catalog_track_id ON song_catalog(track_id);
CREATE INDEX idx_song_catalog_artist ON song_catalog(artist);
CREATE INDEX idx_song_catalog_popularity ON song_catalog(total_attempts DESC, unique_users_attempted DESC);
CREATE INDEX idx_song_catalog_success_rate ON song_catalog(success_rate DESC);
CREATE INDEX idx_song_match_events_track_id ON song_match_events(track_id);
CREATE INDEX idx_song_match_events_created_at ON song_match_events(created_at);
CREATE INDEX idx_song_match_events_success ON song_match_events(success);
CREATE INDEX idx_song_match_events_user_id ON song_match_events(user_id);

-- Trigger to update song catalog stats when karaoke session changes
CREATE TRIGGER update_song_stats_on_session_complete
...
CREATE TRIGGER update_song_catalog_timestamp 
